...
function floorAndGreedyIntegerize(A, supply, prices, xFrac){
  ...
  while(steps++ < MAX_ADD){
    ...
    for(let j=0;j<n;j++){
      ...
      // Utilization bonus: prefer consuming resources that still have a lot remaining (reduces leftover).
      let util = 0, k = 0;
      for(let i=0;i<m;i++){
        const a = A[i][j];
        if(a<=0) continue;
        util += Math.min(1, rem[i] / Math.max(1e-9, supply[i]));
        k++;
      }
      if(k>0) util /= k;
      const base = prices[j] / Math.max(1e-9, cost);
      const score = base * (1 + 0.35 * util);
      if(score > bestScore){
        bestScore = score;
        best = j;
      }
