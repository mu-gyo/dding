...
function floorAndGreedyIntegerize(A, supply, prices, xFrac){
  ...
  while(steps++ < MAX_ADD){
    ...
    for(let j=0;j<n;j++){
      if(!fits(j)) continue;
      // Greedy tie-breaker for better utilization (B mode):
      // 1) maximize unit price first (revenue priority)
      // 2) then prefer items that consume currently-abundant resources (reduce leftover)
      // 3) then prefer lower scarcity-weighted cost (more "efficient" use of remaining resources)
      let abundantUse = 0;
      let cost = 0;
      for(let i=0;i<m;i++){
        const a = A[i][j];
        if(a<=0) continue;
        abundantUse += a * Math.max(0, rem[i]);          // higher if it consumes what we have a lot of
        cost += a / Math.max(1, rem[i]);                // higher if it consumes scarce resources
      }
      // Lexicographic score packed into a single number:
      // price dominates, then abundantUse, then efficiency(1/cost).
      const efficiency = 1 / Math.max(1e-9, cost);
      const score = (prices[j] * 1e9) + (abundantUse * 1e3) + efficiency;
      if(score > bestScore){
        bestScore = score;
        best = j;
      }
