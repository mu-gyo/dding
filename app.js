...
/* =========================================================
 * PATCH: TAB2 Worker offload (stable, no UI freeze)
 * - Keeps math/results identical
 * - Prevents "응답 없음" by moving Tier-ILP brute-force to Worker
 * - Does NOT modify existing optimizeActual() / handlers directly.
 *   Instead, installs a CAPTURE-PHASE click handler that overrides the old one.
 * ========================================================= */
(function(){
  // Only run in main thread (document exists)
  if(typeof document === "undefined") return;

  function runTierILPInWorker(names, unit, baseState, recipes, midSet, finalSet){
    return new Promise((resolve, reject)=>{
      try{
        const w = new Worker("tab2_worker.js");
        w.onmessage = (e)=>{
          const d = e.data || {};
          w.terminate();
          if(d.ok) resolve(d.out);
          else reject(new Error(d.error || "worker failed"));
        };
        w.onerror = (err)=>{
          w.terminate();
          reject(err);
        ...
        w.postMessage({
          ...
          baseState,
          recipes,
          midItems: Array.from(midSet || []),
          finalItems: Array.from(finalSet || [])
        });
      }catch(err){
        reject(err);
      }
  ...
  async function optimizeActualAsync(){
    // === This is the same logic as optimizeActual(), but tier solving is awaited from worker ===
    if(typeof updateTotalsActual === "function") updateTotalsActual();

    const premiumLevel = Number(document.getElementById("premiumLevel")?.value || 0);
    const premiumMul = (typeof premiumMulFromLevel === "function") ? premiumMulFromLevel(premiumLevel) : 1.0;

    let prices = (typeof PRODUCTS !== "undefined" ? PRODUCTS : []).map(p=> Math.round((p.base||0) * premiumMul));
    if(typeof equalizePricesWithinTierMax === "function"){
      prices = equalizePricesWithinTierMax(prices);
    }

    const recipes = (typeof getAllRecipesForMid === "function") ? getAllRecipesForMid() : {};
    const midSet = new Set((typeof MID_ITEMS !== "undefined") ? MID_ITEMS : []);
    const finalSet = new Set((typeof PRODUCTS !== "undefined" ? PRODUCTS : []).map(p=>p.name));

    const baseState = (typeof _tab2_stateFromUI_noCredit === "function")
      ? _tab2_stateFromUI_noCredit()
      : {fish:{}, inv:{}, inv0:{}};
    ...
    const yFinal = Array((typeof PRODUCTS !== "undefined" ? PRODUCTS.length : 0)).fill(0);
    const craftLogAll = {};
    let stAfter = (typeof _tab2_cloneState === "function") ? _tab2_cloneState(baseState) : JSON.parse(JSON.stringify(baseState));

    for(const grp of tierGroups){
      const names = grp.map(i => PRODUCTS[i].name);
      const unit  = grp.map(i => Math.max(0, Math.floor(Number(prices[i]||0))));

      const sol = await runTierILPInWorker(names, unit, stAfter, recipes, midSet, finalSet);
      const best = sol.best;
    ...
    // usedFish (same as app..js logic)
    const fishSupplyArr = (typeof FISH_ROWS !== "undefined" ? FISH_ROWS : []).map(f=> baseState.fish[f]||0);
    const usedFish = (typeof FISH_ROWS !== "undefined" ? FISH_ROWS : []).map((f,i)=>{
      const sup = Math.max(0, Math.floor(Number(fishSupplyArr[i]||0)));
      const rem = Math.max(0, Math.floor(Number(stAfter.fish[f]||0)));
      return Math.max(0, sup - rem);
    });

    try{ window.LAST_ACTUAL = { y: yFinal, prices, fishSupply: fishSupplyArr }; }catch(e){}

    if(typeof renderActualResult === "function"){
      renderActualResult(yFinal, prices, fishSupplyArr, usedFish);
    }

    // 하위 제작 표 (기존 함수가 있으면 그대로 사용)
    try{
      if(typeof MID_SECTIONS !== "undefined" && typeof recipeYield === "function" && typeof renderNeedCraftTableTo === "function"){
        const rows = [];
        for(const sec of MID_SECTIONS){
          for(const name of (sec.items||[])){
            const crafts = Math.max(0, Math.floor(Number(craftLogAll[name]||0)));
  ...
  // Install CAPTURE-PHASE click handler to override existing one
  const btn = document.getElementById("btnSolveActual");
  if(!btn) return;

  btn.addEventListener("click", async (e)=>{
    // Stop old handler (added earlier) from running
    e.stopImmediatePropagation();
    e.preventDefault();

    try{
      if(typeof setButtonLoading === "function") setButtonLoading(btn, true, "계산 중…");
      if(typeof showGlobalLoadingOverlay === "function") showGlobalLoadingOverlay();

      // Let UI paint first
      await new Promise(r => requestAnimationFrame(()=>setTimeout(r,0)));

      await optimizeActualAsync();
    }catch(err){
      console.error(err);
      alert("탭2 계산(Worker) 중 오류가 발생했습니다.\n" + (err && err.message ? err.message : err));
    }finally{
      if(typeof hideGlobalLoadingOverlay === "function") hideGlobalLoadingOverlay();
